...
def recevoir_logs():
    ...
    if mdp == Mot_de_passe:
        content = request.args.get('content')  # Contenu des logs
        if content:
            try:
...
def renvoyer_tout_les_logs():
    ...
    if mdpa == Mot_de_passe_admin:
        ...
        if logs is not None:
            ...
            logs_simplified = []
            for log in logs:
                logs_simplified.append(f"IP: {log['IP']} | Date: {log['Date']} \n| Niveau: {log['Niveau de sévérité']}")
