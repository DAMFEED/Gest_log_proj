...
def envoyer_logs(mdp, logs):
    ...
    url = f'{BASE_URL}/upload/{json.dumps(logs)}'
    response = requests.get(url, params={'mdp': mdp})
...
def recuperer_logs(mdp, ip):
    ...
    response = requests.get(url, params={'mdp': mdp, 'ip': ip})
...
def recuperer_tout_les_logs(mdpa):
    ...
    response = requests.get(url, params={'mdpa': mdpa})
